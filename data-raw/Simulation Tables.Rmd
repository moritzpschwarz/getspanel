...
sum_tab %>% 
  ...
  select(-test.lev) %>% 
  kable(format = "latex", col.names = c("Sample", "# Regressors", "0.01", 
                                        "0.05","0.01", "0.05")) %>% 
...
sum_tab %>% 
  ...
  select(-dist) %>% 
  kable(format = "latex", col.names = c("Sample", "Test Level","p-value", "1","5","10")) %>% 
  kable_styling(full_width = FALSE) %>% 
  kable_paper() %>% 
...
for(i in 1:nrow(plot_specs) ){
  ...
  df_to_table %>% 
    ...
    rename(` ` = type) %>%  
    kable(format = "latex",
          col.names = c("AR", "Out Prop.", "Test Level","p-value","# Regressors","Type", "2","3","4","6")) %>% 
    kable_styling(full_width = FALSE) %>% 
    kable_paper() %>% 
    pack_rows("Sample Size: 100", 1, 2) %>% 
    pack_rows("Sample Size: 200", 3, 4) %>% 
    pack_rows("Sample Size: 300", 5, 6) %>% 
    pack_rows("Sample Size: 400", 7, 8) %>% 
    pack_rows("Sample Size: 500", 9, 10) %>% 
    pack_rows("Sample Size: 100", 11, 12) %>% 
    pack_rows("Sample Size: 200", 13, 14) %>% 
    pack_rows("Sample Size: 300", 15, 16) %>% 
    pack_rows("Sample Size: 400", 17, 18) %>% 
    pack_rows("Sample Size: 500", 19, 20) %>% 
