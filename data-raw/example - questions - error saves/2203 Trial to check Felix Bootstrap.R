...
# quick test to respond to Felix' email
# DELETE AGAIN WHEN DONE
a <- specs[specs$hypothesis == "null" & specs$bootstrap == TRUE & specs$dist == "t3" &
             specs$ar == 0 & specs$sample == 100 & specs$boot.pval.scale == 1 &
             specs$clean.sample == TRUE,]
...
a$sample <- NULL
specs_old <- specs
specs <- dplyr::full_join(a, data.frame(sample = 30:60), by=character())
specs$id.seq <- specs$sample
# save(specs, file = here("data-raw", "bootstrap_test","simulations", "spec_list.RData"))
#for (j in new_order_final$id.seq){
for (j in 1:nrow(specs)){

  #j <- 1
  print(j)
  if(file.exists(here("data-raw", "bootstrap_test", "simulations",paste0(paste(specs[j,],collapse = "_"),".RData")))){next}
  if(file.exists(here("data-raw", "bootstrap_test", "simulations",paste0("Running_",j,".txt")))){
    next
  } else {
    file.create(here("data-raw", "bootstrap_test", "simulations",paste0("Running_",j,".txt")))
  }
  ...
  save(res, file = here("data-raw", "bootstrap_test", "simulations",paste0(paste(specs[j,],collapse = "_"),".RData")))
  unlink(here("data-raw", "bootstrap_test", "simulations",paste0("Running_",j,".txt")))
} #j loop  closed
