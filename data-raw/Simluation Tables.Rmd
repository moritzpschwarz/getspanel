...
sum_tab %>% 
  ...
  add_header_above(c(" " = 4, "AR" = 3))  -> tab3 
...
for(i in 1:nrow(plot_specs) ){
  ...
  df_to_table %>% 
    kable(format = "latex", 
          ...
          escape = FALSE,
          digits = 3,
          col.names = c("n","AR", "Outl. Prop.", "Level",
...
datnull %>% 
  ...
  select(dist:p_alpha, contains("Raw"), 
         contains("Cleaned_1"),contains("Cleaned_2"), everything()) %>% 
  ...
  kable(
    ...
    col.names = c("n","\\# Regressors", "AR", "Level","$\\gamma_c$",
                  rep(c("Asymp.","$L_1$","$L_2$"),3)), 
    escape = FALSE
  ) %>% 
  kable_paper() %>% 
  add_header_above(c(" " = 5, "Raw" = 3, "Clean" = 3, "Clean Scaled" = 3)) %>% 
  add_header_above(c(" " = 5, "Rejection Frequency" = 9)) %>% 
...
for(i in c(2,3, 4, 6)){
  ...
  sum_tab %>% 
    ...
    kable(
      format = "latex", booktabs = TRUE, 
      col.names = c("$\\lambda$", "n","\\# Regressors", "AR", "Level","$\\gamma_c$",
                    rep(c("Asymp.","$L_1$","$L_2$"),3)), 
      escape = FALSE
    ) %>% 
    kable_paper() %>% 
    add_header_above(c(" " = 6, "Raw" = 3, "Clean" = 3, "Clean Scaled" = 3)) %>% 
    add_header_above(c(" " = 6, "Rejection Frequency" = 9)) %>% 
