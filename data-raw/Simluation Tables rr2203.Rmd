...
for(i in c(2, 4, 6)){
  sum_tab %>% 
    ...
    select(dist, lambda, sample, clean.sample, nreg, ar,
           test.lev, p_alpha, parametric, rej, 
           rej.L1.boot, rej.L2.boot, rej.var.boot) %>% 
    arrange(dist, test.lev, sample) %>% 
    
    mutate(clean.sample = ifelse(clean.sample, "Cleaned","Raw"), 
           parametric = ifelse(parametric, "Parametric","Non-Parametric")) %>% 
    
    pivot_wider(id_cols = c(dist, lambda, sample, nreg, ar, 
                            test.lev, p_alpha, parametric),
                names_from = c(clean.sample),
                values_from = c(rej, rej.L1.boot, rej.L2.boot, rej.var.boot)) %>% 
    ...
    select(-dist) %>% 
    arrange(ar, test.lev, sample) %>% 
    kable(
      format = "latex", booktabs = TRUE, 
      col.names = c("$\\lambda$", "n","\\# Regressors", "$\\rho$", "Level","$\\gamma_c$","Parametric"," ",  rep(c("$L_1$","$L_2$", "Variance"),1)), 
      escape = FALSE
    ) %>% 
    kable_paper() %>% 
    add_header_above(c(" " = 7, "Asymp." = 1, "Clean" = 3)) %>% 
    add_header_above(c(" " = 7, "Rejection Frequency" = 4)) %>% 
    
    pack_rows("Normal Distribution",1, 16) %>%
    #pack_rows("$t_3$ Distribution", 9, 16, escape = FALSE) %>% 
    return() -> tab_boot_alt_intermed
  
  
  column_tables(list(tab_boot_alt_intermed), 
                caption = paste("Simulation performance under the alternative for varying sample sizes when using the bootstrap implementations of our test (for outlier magnitude of ",i,"SD of the error term) using Parametric Bootstraps."), 
                label = paste0("fig_out_sim_alt_boot_table_param",i), resize = TRUE) %>% 
    
    paste0(table_list_boot_alt_param, ., sep = "\n") -> table_list_boot_alt_param
  
}
```

```{r}
cat(table_list_boot_alt_param, file = here("data-raw/text/v8/rr2203/table_boot_alternative_param.tex"))
```
...
for(i in c(2, 4, 6)){
  sum_tab %>% 
    ...
    # copy the value for rej for t3 because we never ran a clean run
    ...
    arrange(dist, ar, test.lev, sample) %>% 
    select(-dist) %>% 
    
    kable(
      format = "latex", booktabs = TRUE, 
      col.names = c("$\\lambda$", "n","\\# Regressors", "$\\rho$", "Level","$\\gamma_c$","Parametric"," ",  rep(c("$L_1$","$L_2$", "Variance"),2)), 
      escape = FALSE
    ) %>% 
    kable_paper() %>% 
    add_header_above(c(" " = 7, "Asymp." = 1, "Raw" = 3, "Clean" = 3)) %>% 
    add_header_above(c(" " = 7, "Rejection Frequency" = 7)) %>% 
    
    pack_rows("Normal Distribution",1, 16) %>%
    pack_rows("$t_3$ Distribution", 17, 32, escape = FALSE) %>% 
    return() -> tab_boot_alt_intermed
  
  
  column_tables(list(tab_boot_alt_intermed), 
                caption = paste("Simulation performance under the alternative for varying sample sizes when using the bootstrap implementations of our test (for outlier magnitude of ",i,"SD of the error term) using Non-Parametric Bootstraps."), 
                label = paste0("fig_out_sim_alt_boot_table_nonparam",i), resize = TRUE) %>% 
    
    paste0(table_list_boot_alt_nonparam, ., sep = "\n") -> table_list_boot_alt_nonparam
  
}
```

```{r}
cat(table_list_boot_alt_nonparam, file = here("data-raw/text/v8/rr2203/table_boot_alternative_nonparam.tex"))
```
...
for(i in c(2, 4, 6)){
  ...
  column_tables(list(tab_boot_alt_intermed), 
                caption = paste("Simulation performance under the alternative for varying sample sizes when using the bootstrap implementations of our test for a Bad Leverage Point (outlier magnitude of ",i,"SD of the error term) using Non-Parametric Bootstraps."), 
                label = paste0("fig_out_sim_alt_boot_table_BadLeverage",i), 
                resize = TRUE) %>% 
    
    paste0(table_list_boot_alt_BL, ., sep = "\n") -> table_list_boot_alt_BL
  
}
```

```{r}
cat(table_list_boot_alt_BL, file = here("data-raw/text/v8/rr2203/table_boot_alternative_BadLeverage.tex"))
```
