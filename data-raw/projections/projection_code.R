...
source("C:/Users/Felix/OneDrive/Documents/Projects/IS beta testing/code/distorttest_v5.R")

rel.coef <- c("temp", "temp2", "temp_loggdp", "temp2_loggdp")
rel.coef.lag <- c("temp", "temp2", "temp_L1.loggdp", "temp2_L1.loggdp")


dist1 <- distorttest(m1.isat,  coef=rel.coef)
dist1.lag <- distorttest(m1.isat.lag,  coef=rel.coef.lag)

coefficients(dist1.lag$ols)[1:10]
coefficients(m1.arx.lag)[1:10]

coefficients(dist1.lag$ols)[rel.coef.lag]

outliertest(m1.isat.lag)
outliertest(m1.isat)
...
print(xtable(res.tab, type = "latex"),
      file = "C:/Users/Felix/OneDrive/Documents/Projects/IS beta testing/application/adaptation damage/results1.tex"
...
print(xtable(res.tab.lag, type = "latex"),
      file = "C:/Users/Felix/OneDrive/Documents/Projects/IS beta testing/application/adaptation damage/results1_lag.tex"
      , include.rownames=FALSE)
...
ctry_index.com$is.index <- seq(1:NROW(ctry_index.com))
...
isdat1.lag <- isatdates(m1.isat.lag)
...
ctry_outl <- aggregate(ctry_index.com$iis, by=list(ctry_index.com$country), FUN=sum)
...
ctry_outl.lag <- aggregate(ctry_index.com.lag$iis, by=list(ctry_index.com.lag$country), FUN=sum)
...
ctry_index.com$country[ctry_index.com$country=="ROM"] <- "ROU"
ctry_index.com$country[ctry_index.com$country=="ZAR"] <- "COD"

ctry_index.com$continent <- countrycode(sourcevar = ctry_index.com$country,
                                        origin = "iso3c",
                                        destination = "continent")

ctry_index.com$continent[ctry_index.com$country=="ZAR"] <- "Africa"
ctry_index.com$continent[ctry_index.com$country=="ROM"] <- "Europe"

ctry_index.com.lag$country[ctry_index.com.lag$country=="ROM"] <- "ROU"
ctry_index.com.lag$country[ctry_index.com.lag$country=="ZAR"] <- "COD"

ctry_index.com.lag$continent <- countrycode(sourcevar = ctry_index.com.lag$country,
                                            origin = "iso3c",
                                            destination = "continent")

ctry_index.com.lag$continent[ctry_index.com.lag$country=="ZAR"] <- "Africa"
ctry_index.com.lag$continent[ctry_index.com.lag$country=="ROM"] <- "Europe"
...
ctry_index_wide <- dcast(ctry_index.com, country + continent ~ year, value.var="iis_sign")
ctry_index_wide

ctry_index_wide_lev <- dcast(ctry_index.com, country + continent ~ year, value.var="iis_lev")
ctry_index_wide_lev
...
row.names(ctry_index_wide.mat) <- ctry_index_wide.ord$country

ctry_index_wide.mat_lev <- as.matrix(ctry_index_wide.ord_lev[,3:NCOL(ctry_index_wide_lev)])
row.names(ctry_index_wide.mat_lev) <- ctry_index_wide.ord_lev$country
...
#heatmap(data, Colv = NA, Rowv = NA, scale="column")
ctry_index_wide.lag <- dcast(ctry_index.com.lag, country + continent ~ year, value.var="iis_sign")
ctry_index_wide.lag

ctry_index_wide_lev.lag <- dcast(ctry_index.com.lag, country + continent ~ year, value.var="iis_lev")
...
row.names(ctry_index_wide.mat.lag) <- ctry_index_wide.ord.lag$country

ctry_index_wide.mat_lev.lag <- as.matrix(ctry_index_wide.ord_lev.lag[,3:NCOL(ctry_index_wide_lev.lag)])
row.names(ctry_index_wide.mat_lev.lag) <- ctry_index_wide.ord_lev.lag$country
...
coef_m2.isat <- coefficients(m1.isat)
coef_m2.isat.lag <- coefficients(m1.isat.lag)
