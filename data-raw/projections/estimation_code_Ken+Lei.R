...
dat <- vroom(file = here("damage_curve_country_dataset_timetrends_updated02-19.csv"))
...
for(target_pval in c(0.05, 0.01, 0.001)){
  ...
  # in the first step, we are running a model with all variables to be able to isolate the perfectly collinear variables
  # in the lm model they will return NA coefficients
  dat %>%
  ...
  # we now isolate these NA coefficients
  m1 %>%
    tidy %>%
  ...
  dat %>%
    select(iso, year, diff.ln_gdp_cap, temp, temp_2, prcp, prcp_2 , starts_with(c("year_","time_", "iso_"))) %>%
    # in this step, we are dropping the NA coefficients
    select(-all_of(m1_drop)) %>%
    drop_na -> m2_data

  # this gives us our final data for m2
  write_csv(m2_data, here("data-raw/projections/m2_data.csv"))

  # we run m2 with the final data
  ...
  # now we run a Huber-Skip consistent outlier robust estimation in parallel
  # the function that we use is IIS (impulse indicator saturation) from the gets package
  # this also directly returns the Jiao-Pretis-Schwarz test statistic (use distorttest() or read at the bottom of the printed result)
  if(execute_isat){
    m2.isat <- isat(
  ...
  # but for the adaptation models, we are also including variables that are interacted with GDP per capita
  dat %>%
    ...
    # this just moves the interaction terms to the front of the dataset
