...
matrix("",
       ncol = length(c("r.squared", "adj.r.squared", 
                       ...
                       "df.residual", "nobs", "Outlier Distortion test-stat", "Outlier Distortion p-value"))) %>% 
  as_tibble(.name_repair = "minimal") %>% 
  setNames(c("r.squared", "adj.r.squared", 
             "sigma", "statistic", "p.value", "df", 
             "logLik", "AIC", "BIC", "deviance", 
             "df.residual", "nobs","Outlier Distortion test statistic", "Outlier Distortion p-value")) %>% 
  mutate(`Outlier Distortion test statistic` = distorttest(m2.isat)$statistic,
         `Outlier Distortion p-value` = distorttest(m2.isat)$p.value)-> gl1
...
matrix("",
  ...
  setNames(c("r.squared", "adj.r.squared", 
             ...
             "logLik", "AIC", "BIC", "deviance", 
             "df.residual", "nobs","Outlier Distortion test statistic", "Outlier Distortion p-value")) %>% 
  mutate(`Outlier Distortion test statistic` = distorttest(am2.isat)$statistic,
         `Outlier Distortion p-value` = distorttest(am2.isat)$p.value)-> gl2
