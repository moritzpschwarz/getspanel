...
create_maps <- function(data, prcp = FALSE){
  ...
  LevelData %>%
    ...
    mutate(pc.5 = ifelse(sig.90 != 1, NA, pc.5)) %>%
    select(-starts_with("sig.")) %>%

    pivot_wider(id_cols = c(baseline, model, scenario, map_position, iso, region, final_temp),
                names_from = specification, values_from = pc.5) %>%
    mutate(Difference = IIS - Base) %>%
    pivot_longer(cols = c(Base, IIS, Difference), names_to = "row", values_to = "pc.5") %>%

    full_join(world_df, by = "region") -> IISBase_diff_Data
  ...
  default_ggplot_options <- list(geom_polygon(aes(x=long,y=lat,group=group),fill="grey",inherit.aes = FALSE, data = world_df),
                                 ...
                                 scale_fill_gradientn(colours = brewer.pal(name = "RdBu",n=11),
                                                      ...
                                                      guide = guide_colourbar(title = "GDP per capita Difference to Baseline",
                                                                              title.position = "top"),
                                 ...
                                 theme(legend.position = "bottom",
                                       ...
                                       legend.text = element_text(family = "Myriad Pro",size=8)))
  ...
  IISBase_diff_Data %>%
    ...
    ggplot() +
    facet_grid(row~model)+
    default_ggplot_options +
    #labs(title = "Country-level Project between Base and IIS",
    #     subtitle = "Large Changes in Northern Hemisphere countries due to high projected increase of GDP.") +
    NULL -> plot

  ggsave(plot, filename = here(paste0("data-raw/projections/out/Map_IISBase",ifelse(prcp,"_withprcp","_noprcp"),"_diff.pdf")),
                               device = cairo_pdf,height = 5,width = 6)
...
with_prcp <- read_csv(here("data-raw","projections",paste0("all_models_iso_quantiles_sq.csv")))
create_maps(with_prcp, prcp = TRUE)


no_prcp <- read_csv(here("data-raw","projections",paste0("noprcp_all_models_iso_quantiles_sq.csv")))
create_maps(no_prcp, prcp = FALSE)
