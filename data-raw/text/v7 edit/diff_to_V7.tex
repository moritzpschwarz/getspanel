...
\DIFaddend Once we control for these outliers (some of which coincide with the first Gulf War and collapse of the Soviet Union), the estimated impacts of temperatures on economic growth are attenuated at all income levels compared to conventional OLS estimates. \DIFaddbegin \DIFadd{Using these estimates, we then compare the effect of the robust estimation in projections of climate impacts to the end of the century under different temperature scenarios. Projection results consistently indicate that higher levels of warming could lead to higher impacts on GDP per capita, however, we show that the uncertainty reduces considerably when using robust estimation.
}\DIFaddend



\textbf{NOTE \DIFaddbegin \DIFadd{for MORITZ}\DIFaddend : One sentence here on what this means for the projections.}

\textbf{NOTE \DIFaddbegin \DIFadd{for MORITZ}\DIFaddend : Do we need to describe our application more so as to put more weight on climate side?}
...
We show the estimated non-linear relationship between temperatures and growth for three different income levels (\DIFdelbegin \DIFdel{25th, 50th, and 75th }\DIFdelend \DIFaddbegin \DIFadd{25\textsuperscript{th}, 50\textsuperscript{th}, and 75\textsuperscript{th} }\DIFaddend income percentiles) in Figure \ref{fig_dist_app1}. Results across income levels show that controlling for outlying observations dampens the impacts of temperatures onto economic growth. Using robust estimates, the projected negative temperature impacts are less steep (but still negative) for low income countries, and dampened for middle income countries. For rich countries, the positive effects of higher temperatures are drastically weakened, suggesting attenuated positive effects.

Beyond distortion in coefficients, we show the distribution of the detected outlying observations in the panel model over time, countries, and both (Figures \ref{fig_out_app1} and \ref{fig_map_app1}). The number of detected outliers greatly exceeds the number expected by chance. In total, \DIFdelbegin \DIFdel{150 }\DIFdelend \DIFaddbegin \DIFadd{165 and 170 }\DIFaddend country-year observations \DIFaddbegin \DIFadd{were }\DIFaddend identified as outlying compared to \DIFdelbegin \DIFdel{70.7 }\DIFdelend \DIFaddbegin \DIFadd{77.16 }\DIFaddend expected under the null when there are no outliers. The observed proportion of outliers \DIFaddbegin \DIFadd{\ignore{CHECK!!}}\DIFaddend $\hat{\gamma}_{c=2.57}=$ 150/7007=0.021 is statistically different from the expected proportion of $\gamma_{c=2.57}=0.01$ ($p<0.001$ using the \DIFdelbegin \DIFdel{Jiao and Pretis REF }\DIFdelend \DIFaddbegin \citet{jiao2020testingoutlier}\DIFadd{\ignore{Check this reference} }\DIFaddend proportion test comparing 0.021 to 0.01). Further, the distribution of outliers does not appear to be random over time and space -- outliers are concentrated in developing countries and there is a temporal cluster coinciding with the collapse of the Soviet Union \DIFaddbegin \DIFadd{and the onset of the Gulf War}\DIFaddend , which may well capture an idiosyncratic shock not modelled in the base panel model.

Overall, our results provide macro-econometric evidence of income-driven adaptation -- temperatures have a non-linear effect on GDP per capita growth, but this relationship varies by income, where poor countries face negative impacts of warmer temperatures while rich countries face predominantly positive impacts, with middle-income countries seeing the familiar quadratic relationship. Moreover, our estimation results highlight the importance of controlling for outlying observations and testing for subsequent distortion. Idiosyncratic shocks in the model distort coefficient estimates, with robust results showing dampened climate impacts onto GDP per capita growth compared to conventional OLS estimates \DIFaddbegin \DIFadd{(see Figure \ref{fig_projection})}\DIFaddend .
...
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{When we project the estimated relationships to the end of the century, our results indicate high global reductions in median GDP per capita when compared to the baseline (see Figure \ref{fig_projection} and Table \ref{tab_app2_project}).
}\DIFaddend

\DIFdelbegin %DIFDELCMD < \clearpage
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{\ignore{CHECK}
...
\DIFdelbegin %DIFDELCMD < \begin{table}[ht]  \centering
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \input{application_main_table.tex}
\DIFaddend
...
%DIFDELCMD < %%%
\DIFdelendFL %DIF > \vspace{-.35in}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.65]{coef_v2_lag.pdf}
%DIFDELCMD < %%%
\DIFdelendFL %DIF > \includegraphics[scale=0.65]{coef_v2_lag.pdf}
...
%DIFDELCMD < %%%
\DIFdelendFL %DIF > \includegraphics[scale=0.55]{eff_v1_lag.pdf}
\DIFaddbeginFL \includegraphics[width = \textwidth]{eff.adapt.v2.pdf}
\DIFaddendFL \caption{Estimated Impact of Temperatures on GDP Per Capita Growth Allowing for Adaptation and Controlling for Outliers. OLS-estimated relationship shown in blue, robust IIS estimated relationship shown in red. Estimated non-linear impact function for \DIFdelbeginFL \DIFdelFL{different }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the overall base model (top left) and at }\DIFaddendFL three different income levels (\DIFdelbeginFL \DIFdelFL{middle }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{other top }\DIFaddendFL panels)\DIFdelbeginFL \DIFdelFL{and the observed country level temperatures (bottom panels): countries in at the 25th income percentile, countries with median income, and countries at the 75th income percentile}\DIFdelendFL . Observed temperatures \DIFaddbeginFL \DIFaddFL{for the entire sample and }\DIFaddendFL across income ranges are shown in the \DIFdelbeginFL \DIFdelFL{lower }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{bottom }\DIFaddendFL panel. }
\label{fig_dist_app1}
\end{figure}
...
\begin{figure}[!htbp]  \vspace{-.35in}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.7]{heat1_lag_v1_edited.pdf}
%DIFDELCMD < %%%
\DIFdelendFL %DIF > \includegraphics[scale=0.7]{heat1_lag_v1_edited.pdf}
\DIFaddbeginFL \includegraphics[width = \textwidth]{heat1_adapt.pdf}
\DIFaddendFL \caption{Detected Outliers using the robust IIS estimator across countries and time in the global cross-country panel from \DIFdelbeginFL \DIFdelFL{1962-2013}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1961-2017 in the adaptation model}\DIFaddendFL . Top Figure panel shows the number of positive (red) and negative (blue) outliers aggregated over countries in each year. Bottom panel shows country-year observations as gray when not outlying, blue when there is a negative outliers, and red for positive outliers.}
\label{fig_out_app1}
\end{figure}

\begin{figure}[!htbp]  \vspace{-.35in}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.6]{ctry_map_lag_edited.pdf}
%DIFDELCMD < %%%
\DIFdelendFL %DIF > \includegraphics[scale=0.6]{ctry_map_lag_edited.pdf}
\DIFaddbeginFL \includegraphics[width = \textwidth]{ctry_map_adapt.pdf}
\DIFaddendFL \caption{Detected outliers aggregated over the full sample \DIFaddbeginFL \DIFaddFL{of 1961 - 2017 }\DIFaddendFL by \DIFdelbeginFL \DIFdelFL{contry }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{country }\DIFaddendFL in the panel \DIFaddbeginFL \DIFaddFL{in the adaptation model}\DIFaddendFL . Gray denotes no outliers detected.}
\label{fig_map_app1}
\end{figure}


\DIFaddbegin \begin{figure}[!htbp]  \vspace{-.35in}
\centering
...
\caption{\DIFaddFL{Projected Climate Impacts for the period 2090 - 2099 and for all CMIP5 models with a mean warming above 4.5\textdegree C. Projection excludes Precipitation Effect. Estimate shown is the median, grey indicates that the 90\% confidence interval spans 0.}}
...
%DIF > \includegraphics[scale=0.6]{ctry_map_lag_edited.pdf}
\includegraphics[width = \textwidth]{Map_IISBase_noprcp_diff.pdf}
\caption{\DIFaddFL{Projected Climate Impacts for the period 2090 - 2099 and for all CMIP5 models with a mean warming above 4.5\textdegree C. Projection includes Precipitation Effect. Estimate shown is the median, grey indicates that the 90\% confidence interval spans 0.}}
\label{fig_projection_map}
