...
for (l in files){
  ...
  #load(here("data-raw/simulations/rr2305/1_t3_30_0.1_50_5_0.05_TRUE_3_20_1_TRUE_TRUE_alternative_4_1_0.5_TRUE_TRUE_1772_1.RData"))
...
for(boot in c(FALSE)){
  for(hypo in c("null","alternative")){
    ...
    #
    #
    # pdf(here(paste0("data-raw/figures/rr2305/asymp_alt_badleverage.pdf")), width=7, height=10)
    #
    # par(mfrow=c(4,2))
    #
    ...
    plot_alternative_asym_badleverage <- function(sum_tab, ar = 0){
      ...
      datalt <- sum_tab[sum_tab$bootstrap == FALSE &
                          sum_tab$sample > 50 &
                          sum_tab$bad_leverage &
                          sum_tab$test.lev == 0.05 &
                          sum_tab$ar == ar,]


      datalt$specfam <- 0
      datalt$specfam[datalt$lambda==2] <- 1
      datalt$specfam[datalt$lambda==3] <- 2
      datalt$specfam[datalt$lambda==4] <- 3
      datalt$specfam[datalt$lambda==6] <- 4
      ...
      datalt$specfam <- 0
      datalt$specfam[datalt$sample==100] <- 4
      datalt$specfam[datalt$sample==200] <- 5
      datalt$specfam[datalt$sample==300] <- 6
      datalt$specfam[datalt$sample==400] <- 7
      datalt$specfam[datalt$sample==500] <- 8
      ...
      plot(datalt$is.euclid.sc[datalt$specfam==4], datalt$rej[datalt$specfam==4] , lty=1, type="b", ylim=c(0, 1),
           xlim=c(0,1), col=col.lam1, ylab="Null Rejection Frequency",
           xlab="Scaled Euclidian Distance of Coefficients:\nd/max(d)",
           main=main_title)
    ...
    pdf(here(paste0("data-raw/figures/rr2305/asymp_alt_badleverage.pdf")), width=9, height=11)

    par(mfrow=c(2,3))
    plot_alternative_asym_badleverage(sum_tab = sum_tab, ar = 0)
    plot_alternative_asym_badleverage(sum_tab = sum_tab, ar = 0.5)
    ...
    if(boot & hypo == "null"){
      ...
      datnull.boot <- sum_tab[sum_tab$bootstrap==TRUE & sum_tab$hypothesis=="null" & sum_tab$sample > 50,]
      # exclude lognorm
      ...
      pdf(here(paste0("data-raw/figures/rr2305/boot_null_nonparam.pdf")), width=pdf.width, height=pdf.height)
      par(mfrow=c(1,4))
      ...
      # variance
      lines(dat.sub$sample[dat.sub$specfam==1], dat.sub$rej.var.boot[dat.sub$specfam==1 ], lty=1, type="b",   col=col.bootfull, lwd=2)
      ...
      legend(50, 0.8, c("Asym", "Full Data (Variance)","Full Data (L2)",  "Clean Data (L2)"),
             bg=NA, bty = "n", title.adj=-0.03, lty=c(1, 1, 3, 3),
             col=c(col.asym, col.bootfull,col.bootfull, col.bootclean),
             lwd=2,  cex=0.9, seg.len=0.5, pt.cex=0.1,  x.intersp=0.2,  y.intersp=1)
      ...
      # variance
      lines(dat.sub$sample[dat.sub$specfam==4], dat.sub$rej.var.boot[dat.sub$specfam==4 ], lty=1, type="b",   col=col.bootfull, lwd=2)
      ...
      legend(50, 0.8, c("Asym", "Full Data (Variance)", "Full Data (L2)", "Clean Data (L2)"),  bg=NA, bty = "n", title.adj=-0.03,
             lty=c(1, 1, 3, 3), col=c(col.asym, col.bootfull,col.bootfull, col.bootclean),
             lwd=2,  cex=0.9, seg.len=0.5, pt.cex=0.1,  x.intersp=0.2,  y.intersp=1)
      ...
      datnull <- datnull[datnull$test.lev==0.05,] # choose test level
      datnull <- datnull[datnull$parametric==TRUE,] # choose non-parametric

      pdf.width <- 13
      pdf.height <- 5.5

      pdf(here(paste0("data-raw/figures/rr2305/boot_null_param.pdf")), width=pdf.width, height=pdf.height)
      par(mfrow=c(1,4))
      ...
      #       pdf.height <- 5.5
      #
      #       pdf(here(paste0("data-raw/figures/rr2305/boot_null_lognorm.pdf")), width=pdf.width, height=pdf.height)
      #       par(mfrow=c(1,1))
      #
      #       #### Plot 1: Size against number of observations for different levels
      #       col.asym <- "gray55"
    ...
    if(boot & hypo == "alternative"){
      ...
      plot_alternative_boot <- function(sum_tab,
        ...
        datalt.boot <- sum_tab[sum_tab$bootstrap==TRUE&sum_tab$hypothesis=="alternative" &
                                 sum_tab$parametric == parametric & sum_tab$sample > 50,]
        ...
        if(parametric){lines(datalt.boot$sample[datalt.boot$specfam==2], datalt.boot$rej[datalt.boot$specfam==2], lty=1, type="b",   col=col.asym, lwd = 2)}
        # Variance
        lines(datalt.boot$sample[datalt.boot$specfam==1], datalt.boot$rej.var.boot[datalt.boot$specfam==1], lty=1, type="b",  col=col.bootfull, lwd = 2)


        abline(h=0.05, col=col.asym)
        text(x=50, y=0.017, label="0.05", col=col.asym)

        legend(50, 0.7, c("Asymp", "Full Data (Variance)", "Full Data (L2)", "Clean Data (L2)"),  bg=NA, bty = "n", title.adj=-0.03,
               lty=c(1, 1, 3, 3), col=c(col.asym, col.bootfull, col.bootfull, col.bootclean), lwd=2,  cex=1.1, pt.cex=1.1,  x.intersp=0.5,  y.intersp=1)
        ...
        if(!parametric){
          ...
          if(parametric){lines(datalt.boot$sample[datalt.boot$specfam==4], datalt.boot$rej[datalt.boot$specfam==4], lty=1, type="b",   col=col.asym, lwd = 2)}
          # Variance
          lines(datalt.boot$sample[datalt.boot$specfam==4], datalt.boot$rej.var.boot[datalt.boot$specfam==4], lty=1, type="b",   col=col.bootfull, lwd = 2)
      ...
      pdf(here(paste0("data-raw/figures/rr2305/boot_alt_nonparam.pdf")), width=12, height=10)

      par(mfrow=c(3,4))

      plot_alternative_boot(sum_tab = sum_tab, lambda = 2, parametric = FALSE)
      plot_alternative_boot(sum_tab = sum_tab, lambda = 4, parametric = FALSE)
      plot_alternative_boot(sum_tab = sum_tab, lambda = 6, parametric = FALSE)


      dev.off()

      # Removed in July 2022
      # Reason: The bootstrap under the Null for parametric bootstraps doesn't work very well
      #         Therefore it does not make sense to show the performance under the alternative
      # pdf(here(paste0("data-raw/figures/rr2305/boot_alt_param.pdf")), width=8, height=10)
      #
      # par(mfrow=c(3,2))
      #
...
plot_alternative_boot_badleverage <- function(sum_tab,
  ...
  abline(h=0.05, col=col.asym)
  text(x=60, y=0.017, label="0.05", col=col.asym)
...
pdf(here(paste0("data-raw/figures/rr2305/boot_alt_badleverage.pdf")), width=8, height=5)
