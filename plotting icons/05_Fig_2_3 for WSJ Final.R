...
counterfactual_df %>%
  as_tibble() %>%
  filter(id %in% c("UnitedKingdom", "China", "UnitedStates")) %>%
  mutate(id = case_when(id == "UnitedStates" ~ "United States",
                        id == "UnitedKingdom" ~ "United Kingdom",
                        TRUE ~ id)) %>%
  ...
  writexl::write_xlsx("Plot Data for WSJ.xlsx")
...
overall_policies %>%
  ...
  filter((country == "China" & sector == "Industry") |
         (country == "UnitedKingdom" & sector == "Electricity") |
         (country == "UnitedStates" & sector == "Transport")) %>%
  ...
  mutate(country = case_when(country == "UnitedStates" ~ "United States",
                             ...
                             TRUE ~ country),
         name = case_when(!is.na(label) ~ label, TRUE ~ name)) %>%
  drop_na(x) %>%
  select(-label) %>%
...
writexl::write_xlsx(overall_policies_fin, path = "Policy Data for WSJ.xlsx")
