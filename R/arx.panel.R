arx.panel <- function (y, mc = FALSE, ar = NULL, ewma = NULL, mxreg = NULL,
          ...
          LAPACK = FALSE, plot = NULL,time,id,cluster)
{
  vcov.type <- match.arg(vcov.type)
  tmp <- regressorsMean(y, mc = mc, ar = ar, ewma = ewma,
  ...
  if (is.null(user.estimator)) {
    ...
    out <- ols.clustered(aux$y, aux$mX, tol = tol, LAPACK = LAPACK, time,id,cluster,
  ...
  out$diagnostics <- diagnostics(out, ar.LjungB = ar.LjungBarg,
